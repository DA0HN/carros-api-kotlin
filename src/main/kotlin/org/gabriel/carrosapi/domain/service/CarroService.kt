...
import org.gabriel.carrosapi.exceptions.ObjectNotFoundException
...
class CarroService(private val repository: CarroRepository, private val modelMapper: ModelMapper) {
  ...
  fun findById(id: Long): CarroDTO = repository.findById(id)
    .map { buildCarroDTO(it) }
    .orElseThrow { ObjectNotFoundException("Carro não encontrado") }


  fun findByTipo(tipo: String): List<CarroDTO> = repository.findByTipo(tipo)
    .map(this::buildCarroDTO)
